...
from datasets.dataset_covid import Covid_dataset
from datasets.dataset_synapse import Synapse_dataset
from utils import test_single_volume,save_covid_test_result
...
parser.add_argument('--dataset', type=str,
                    default='Covid', help='experiment_name')
...
def inference_covid2d(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        test_image.append(image.squeeze(0).cpu().detach().numpy())
        test_label.append(label.squeeze(0).cpu().detach().numpy())
        ...
        logging.info('idx %d case %s mean_dice %f mean_precision %f mean_recall %f mean_f1 %f mean_hd95 %f' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2], np.mean(metric_i, axis=0)[3], np.mean(metric_i, axis=0)[4]))
    metric_list = metric_list / len(db_test)
    for i in range(1, args.num_classes):
        logging.info('Mean class %d mean_dice %f mean_precision %f mean_recall %f mean_f1 %f mean_hd95 %f' % (i, metric_list[i-1][0], metric_list[i-1][1], metric_list[i-1][2], metric_list[i-1][3], metric_list[i-1][4]))
    performance = np.mean(metric_list, axis=0)[0]
    precision = np.mean(metric_list, axis=0)[1]
    recall = np.mean(metric_list, axis=0)[2]
    f1 = np.mean(metric_list, axis=0)[3]
...
if __name__ == "__main__":
    ...
    if args.is_savenii:
        ...
        test_save_path = None
    inference_covid2d(args, net, test_save_path)
